...
struct ScrollViewConfigurator: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> ScrollViewConfiguratorView {
        let view = ScrollViewConfiguratorView()
        view.configure = configure
        view.searchStrategy = searchStrategy
        return view
    }

    func updateNSView(_ nsView: ScrollViewConfiguratorView, context: Context) {
...
final class ScrollViewConfiguratorView: NSView {
    var configure: ((NSScrollView) -> Void)?
    var searchStrategy: ScrollViewConfigurator.SearchStrategy = .nearestOrFallback
